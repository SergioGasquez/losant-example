...
if [ "${WOKWI_PROJECT_ID}" == "" ]; then
    wokwi-server --chip esp32c3 target/${ESP_ARCH}/${BUILD_MODE}/losant-example
else
    wokwi-server --chip esp32c3 --id ${WOKWI_PROJECT_ID} target/${ESP_ARCH}/${BUILD_MODE}/losant-example
fi